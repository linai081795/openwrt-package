    ...
    case "${git_laqu}" in
        1)
            if git clone -q --no-checkout "$url" "$C"; then
                ...
                # 替换路径中的特定字符串
                grep -rl 'include $(TOPDIR)/feeds/luci/luci.mk' . | xargs -r sed -i 's#include $(TOPDIR)/feeds/luci/luci.mk#include \$(TOPDIR)/feeds/luci/luci.mk#g'
                grep -rl 'include $(TOPDIR)/feeds/packages/lang/' . | xargs -r sed -i 's#include $(TOPDIR)/feeds/packages/lang/#include \$(TOPDIR)/feeds/packages/lang/#g'
                # 移动文件到目标路径
                rm -fr "${content}"
